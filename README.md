...
Configured under `discovery.providers` (list). **If omitted**, defaults are the full list in `recon/core/defaults.py` (**crtsh**, **subfinder**, **waybackurls**, **assetfinder**, **github_subdomains**, **shuffledns**, **massdns**). Add **`amass_passive`** (and install **`amass`**) if you want OWASP Amass. **shuffledns** only emits hosts when **`discovery.wordlist`** and **`discovery.resolvers`** are set. Multiple entries are merged by `CompositeDiscoveryProvider` and deduplicated; the apex domain is injected if missing.
